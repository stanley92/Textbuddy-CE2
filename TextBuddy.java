...
public class TextBuddy {
	...
	private static void searchTexts(Scanner sc, ArrayList<String> texts, String fileName) {
		...
		if (texts.size() == 0) {
			...
			for(int i=0; i<texts.size(); i++) {
				if (texts.get(i).toUpperCase().contains(searchItem.toUpperCase())) {
					System.out.println(i+1 + ". " + texts.get(i));
				}
			}
		}
	...
	private static void deleteText(Scanner sc, ArrayList<String> texts,
		...
		System.out.println("deleted from " + fileName + " \"" + texts.get(deleteNo-1) + "\"");
